...
const sendSetPasswordEmail = async (email, token, firstName) => {
  ...
  const registrationLink = `http://localhost:5173/admin/register/complete/${token}`;
  const mailOptions = {
    ...
    html: `
      <p>Hello ${firstName},</p>
      <p>You've been registered as a Court Admin. Please click <a href="${registrationLink}">here</a> to set your password and complete the registration.</p>
      <p>Thank you,</p>
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    // Send email with password setting link
    await sendSetPasswordEmail(email, token, firstName);

    res.status(201).json({
      message: 'Court admin registered successfully. Please check your email to complete the registration.',
      admin: newAdmin,
    });
  } catch (error) {
    console.error(error);
...
// Complete the registration by setting password
router.post('/register/complete/:token', async (req, res) => {
  try {
    ...
    const admin = await CourtAdmin.findOne({ password_token: token });

    if (!admin) {
      return res.status(404).json({ error: 'Invalid token or admin not found' });
    }

    // Hash the new password using bcrypt
    const hashedPassword = await bcrypt.hash(password, 10);

    // Update admin's password and remove verification token
    admin.password = hashedPassword;
    admin.password_token = undefined;
    await admin.save();
